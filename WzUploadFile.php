...
class WzUploadFile
{
    ...
    public function __construct($post_id, $post_name, $url, $alt)
    {
    ...
    {
        return $this->resolvePattern('%image_alt%');
    }
    ...
    public function downloadImage($url)
    {
        ...
        if (isset($response['response']['code'], $response['body']) && $response['response']['code'] !== 200) {
            return new WP_Error('wz_fail_load', 'Image file bad response.');
        }

        $tempFile = tempnam(sys_get_temp_dir(), 'wz_who');
        file_put_contents($tempFile, $response['body']);
        $mime = wp_get_image_mime($tempFile);
        unlink($tempFile);

        if ($mime === false || strpos($mime, 'image/') !== 0) {
            return new WP_Error('wz_no_img', 'File type is not image.');
        }
        ...
        if (!is_file($image['path'])) {
            return new WP_Error('wz_fail_image_save', 'Image save to upload dir failed.');
        }
    ...
    public function attachImage($image)
    {
        ...
        $attach_id = wp_insert_attachment($attachment, $image['path'], $this->post_id);
